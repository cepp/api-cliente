...
class BuscarPorIdClienteV1ControllerITest {
    ...
    void quandoClienteEncontradoSucesso() throws Exception {
        ...
        this.mvc.perform(get(URI.create("/api/v1/cliente/".concat(idString)))
                        .contentType(MediaType.APPLICATION_JSON)
                        .characterEncoding(StandardCharsets.UTF_8))
                ...
                .andExpect(jsonPath("$.cpf").value("49897572031"))
                .andExpect(jsonPath("$.endereco").value("Rua movimentada agora, 344, AP 2314"))
                .andExpect(jsonPath("$.idade").value(jsonDeserialized.getIdade()))
                .andExpect(jsonPath("$.id").value(idString));
    }

    @Test
    void quandoClienteNaoEncontrado() throws Exception {
        this.mvc.perform(get(URI.create("/api/v1/cliente/".concat("999999")))
                        .contentType(MediaType.APPLICATION_JSON)
                        .characterEncoding(StandardCharsets.UTF_8))
                .andExpect(status().isOk())
